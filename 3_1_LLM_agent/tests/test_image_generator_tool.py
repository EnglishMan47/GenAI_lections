...
"""
Тесты инструмента ImageGeneratorTool

Все обращения к сети подменяются заглушками: тесты должны проходить в GitHub Actions, где нет ни токенов, ни доступа к сервисам генерации, и не тратить бесплатные лимиты аккаунта.
"""
...
# При запуске через pytest путь добавляет conftest.py
...
def make_response(
    status=200, content=FAKE_IMAGE, json_data=None, content_type="image/jpeg", text=""
):
...
class TestImageGeneratorTool(unittest.TestCase):
    ...
    def setUp(self):
        ...
        network = patch(
            ...
            side_effect=AssertionError("Unexpected network call in unit test"),
        )
        network.start()
        self.addCleanup(network.stop)
        settings = patch("llm_agent.tool_image_generator.config", return_value=None)
        settings.start()
    ...
    def test_use_returns_link_to_generated_image(self):
        ...
        # Размер и описание ушли в запрос
    ...
    def test_use_switches_provider_on_http_error(self):
        ...
        with (
            patch("requests.post", side_effect=make_http_error(402)),
        ...
        for error in (
            ...
            with (
                ...
                self.assertIn("pollinations", self.tool.use("cat"))
    ...
    def test_upload_falls_through_to_next_host(self):
        ...
        with patch(
            "requests.post", return_value=make_response(json_data={"files": [None]})
        ):
            self.assertTrue(
                self.tool._save_image(FAKE_IMAGE, "cat", "jpg").startswith("file:///")
            )
        self.assertEqual(len(list(Path(self.temp_dir).glob("*.jpg"))), 3)
    ...
    def test_replicate_generation(self):
        ...
        with patch(
            "requests.post", return_value=make_response(json_data=answer)
        ) as post:
        ...
        with (
            patch("requests.post", return_value=make_response(json_data=started)),
        ...
        for answer in (
            ...
            {"status": "processing", "urls": {"get": "https://untrusted.test/poll"}},
